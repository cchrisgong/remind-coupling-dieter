...
annual_reportCSV = read.csv(paste0(mypath, "capfac", runnumber, "_i", iteration, "_annualreport.csv"), sep = ';', header = T, stringsAsFactors = F)
...
FLHseq0$sorted_x <- rep(seq(1, 8760),90)
...
p1 <- ggplot() +
  ...
  ggsave(filename = paste0(mypath, "screening_curve_capfac", runnumber, "_", iteration, ".png"),  width = 12, height =8, units = "in", dpi = 120)
...
p2<-ggplot() +
  ...
  ggsave(filename = paste0(mypath, "inversescreening_curve_capfac", runnumber, "_", iteration, ".png"),  width = 12, height =8, units = "in", dpi = 120)
  
  FLHseq2 <- FLHseq0 %>% 
    # filter(flh == 1)  %>% 
    select(period, tech, sorted_x, value)
  
  ScreenCurve2 = list(FLHseq2, IC, OM, FC, CO2) %>% 
    reduce(full_join) %>% 
    replace(is.na(.), 0) %>% 
    mutate(value = sorted_x * (FC + CO2)/1e3 + (IC + OM))
  ...
  p3<-ggplot() +
    ...
    coord_cartesian(ylim = c(0,200)) +
    facet_wrap(~period, nrow = 3)
  ggsave(filename = paste0(mypath, "inversescreening_curve_fullyr_capfac", runnumber, "_", iteration, ".png"),  width = 12, height =8, units = "in", dpi = 120)
