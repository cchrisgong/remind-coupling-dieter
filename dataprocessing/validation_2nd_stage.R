...
sorted_files_DT <- paste0(mydatapath, "results_DIETER_i", seq(from = 2, to = length(files_DT_rep), by = 1), ".gdx")
...
get_MARKUPvariable <- function(iteration, MARKUPkey){
  ...
  vrdata0 <- read.gdx(gdx, MARKUPkey,field="l") %>% 
    filter(tall%in%year_toplot) %>%
    filter(all_regi == REGIkey1) %>% 
    mutate(all_te = str_replace(all_te, TECHkeylst_biomass[[1]], plot_RMte_names[[5]])) %>% 
    mutate(all_te = str_replace(all_te, TECHkeylst_biomass[[2]], plot_RMte_names[[5]])) %>% 
...
markup0 <- lapply(iter_toplot_RM, get_MARKUPvariable, MARKUPkey = MARKUP)
...
vrN_REV <- list(vrN_GEN, vrN_PRICE,markup) %>%
  ...
  mutate(MarketValue_RM = MarketValue_RM + markup) %>% 
  select(period,tech,iter,Revenue_RM_billUSD,MarketValue_RM, markup)%>% 
...
write.table(vrN_final, paste0(mypath, "validation_table_", run_number, ".xls"), sep = ";", row.names = F)
