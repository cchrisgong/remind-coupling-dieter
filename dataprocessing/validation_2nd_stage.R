...
iter_tovalid_RM = c(36)
iter_tovalid_DT_4RM = c(37) # the iteration of DIETER needed for reporting iteration=iter_tovalid_RM of remind
iter_tovalid_DT = c(36) # the iteration of DIETER needed for reporting the iteration of DIETER that passes
...
#  get shadow price to be 0 for VRE, all REMIND tech without hc, lig
  ...
  vrN<- list(vrN0,RM_capfac_spread, Adjcost, vrN_grade,vrN1) %>%
    reduce(full_join) %>% 
...
vrN2 <- vrN[, c("iter",	"period",	"tech", "rlf", "REMIND CapFac (%)",	"REMIND CapFac grade (%)", "REMIND Capacity Shadow Price ($/MWh)","REMIND Revenue (billionUSD)","REMIND whole sale electricity price ($/MWh)" ,"REMIND Market value ($/MWh)",	"REMIND Markup ($/MWh)", "DIETER Value factor (%)", "VF_REMIND", "REMIND LCOE ($/MWh)","Marginal adjustment cost ($/MWh)",
