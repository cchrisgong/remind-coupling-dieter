...
# Save as png
ggsave(filename = paste0(outputdir, "/DIETER/FIGURE01.png"),
       bg = "white",
...
plot.remind.total <- out.remind.capacity %>% 
  filter(iter == maxiter-1) %>% 
  filter(period < 2110)%>%
  mutate(period = as.numeric(period))%>% 
...
plot.diff <- list(plot.remind.snap, plot.dieter.snap,plot.remind.total) %>%
  reduce(full_join) %>%
...
p.cap.diff <-ggplot() +
  geom_bar(data = plot.diff ,
           ...
           stat = "identity") +
  scale_fill_manual(name = "Technology", values = color.mapping.cap.order) +
  scale_y_continuous(name = "Difference (GW)", breaks = seq(-20,20,5)) + 
  xlab("Time") +
