...
p.line <- ggplot() + 
  geom_line(data = diff.price.avg.yr,
  ...
  scale_y_continuous(name = "$/MWh",
                     limits = c(0, max(diff.price.avg.yr$value)))
...
# Save as png
ggsave(filename = paste0(outputdir, "DIETER/_FIGURE01.png"),
       bg = "white",
       width = 14,  # Vary width according to how many panels plot has
       height = 10,  # Vary height according to how many panels plot has
       units = "cm")
# Save as svg
ggsave(filename = paste0(outputdir, "DIETER/_FIGURE01.svg"),
       bg = "white",
       width = 14,  # Vary width according to how many panels plot has
       height = 10,  # Vary height according to how many panels plot has
       units = "cm")

# Figure 2: Capacity convergence ------------------------------------------
...
plot.diff <- list(plot.remind.snap, plot.dieter.snap) %>%
  ...
  mutate(delta_cap = (remind_cap - dieter_cap),
         delta_cap.rel = 100*(remind_cap - dieter_cap)/remind_cap)
# Plotting relative differences seems to blow up the difference for technologies
# with low capacities, so maybe rather plot absolute differences.

p.cap.diff <-ggplot() +
  geom_bar(data = plot.diff ,
           mapping = aes(x = period, y = delta_cap, fill = tech),
           stat = "identity") +
  scale_fill_manual(name = "Technology", values = color.mapping.cap.order) +
  scale_y_continuous(name = "Difference (GW)", breaks = seq(-80,80,20)) + 
  xlab("Time") +
...
# Save as png
ggsave(filename = paste0(outputdir, "DIETER/_FIGURE02.png"),
       bg = "white",
